## Preview links — 2026-09-23 (branch `claude/preview-launcher-ltc3uy`)

Cleaner shareable links for the five dreamworlds. `preview.html` at the repo root is a plain HTML/CSS launcher with one link per world. Each link is the game URL with `?start=himalayas` (or `nazca`, `easter`, `pyramid`, `ezekiel`). The boot hook now lives in `sync_html.py` (`PREVIEW_BOOT`, marker-wrapped, placed before the Twine engine on every sync; the old hand-edited copy in the html is removed on sync). `?start=<world>` opens the matching `Preview <World>` passage in the twee (five `[system]` passages after Spend Held Key), which seeds only what the story would have set when the third pillar opened that world (`$inisToldOfPillars`, `$crossedThreshold`, the world's own key `spent`, `$dreamKey` cleared) and goes to the world's first passage. Any other `?start=` value is treated as a passage name. The query string stays in the URL, so a refresh restarts the same world instead of returning to the title. The old `#dss-debug-jump=` hash form is unchanged. Lifetime gifts still record on completion, as they did with the hash form.

Verified in headless Chromium against a local http server (`scratchpad/preview-links-2026-09-23/verify.py`): all five worlds land on the right passage with 0 tw-errors and 0 JS errors, key spent and Inis flag set, sobriety/morale 70; refresh mid-Nazca returns to Nazca Approach; hash form and raw passage names still work; plain load stays on Title; launcher has no horizontal scroll at 390px and its links land. Screenshots beside the script. Prose, variables' meaning, endings and audio untouched.
