...
## Three more rooms: the Colony, the Pillars, Ronnie's — 2026-09-26

Sam's four-hour ask ("the same thing for other venues, search online for an image"). Image sites are blocked from the remote environment, so each room was built from the search descriptions. Same pattern as the French: `#ci-container` at the top of `The Colony Room` (before its `colony-scene` div), `#pi-container` at the top of `Entering The Pillars of Hercules` (before `pillars-scene`), `#ri-container` at the top of `Ronnie Scott's` (before `ronnies-scene`); scene IIFEs "INSIDE THE COLONY", "INSIDE THE PILLARS", "INSIDE RONNIE'S" in the UserScript after the French room and before the Coach block. Hotspots press the passage's own links by wording (`byText`) or its own controls (`#bar-start-btn` at Ronnie's; `.phone-ringing tw-link` everywhere, though the game's fixed ringing modal covers the room and is the real path). Hidden links (`display:none`) are filtered by `getClientRects().length`. The Pillars' third pillar reads the passage's rendered `.pillar.centre` to decide whether it is a trace or solid; it opens "Step through the third pillar" / "Perform the synthesis" when rendered. Tests: `scratchpad/scene-closeups-2026-09-25/{ci,pi,ri}_routes.py OUTDIR WIDTH [reduced]` with `room_harness.py` (run from `scratchpad/audit-2026-09-16`, `PYTHONPATH=.:<that folder>`, server on 8777); step back only after the tween ends and tap the canvas below the fixed header. Pre-existing: the Ronnie's approach scene throws `reading 'trim'` in three.js once after leaving (composer renders after disposal), on main too. Open for Sam: pink card lines and captions; whether the telephone hotspot stays; the remaining venues (Coach and Horses, Trisha's, Lackland's, Chippy, Copper's Lair, O'Flatterly's).

## The French room as navigation — 2026-09-26

Sam: "The bar shouldn't take you to the French passage. You are already in the French. The clicks should give you different paths through it." Done: `#fi-container` now sits inside **The French** (above the French rule); ENTER THE FRENCH's hidden `·` goes straight to The French; `Inside the French` only `(go-to:)`s there (tag `system`). The card lists the passage's currently rendered links as buttons (found by their text with `byText`, since Harlowe 3 links carry no passage name) and pressing one clicks the real link, so all gates and side effects stay in Harlowe. Bar → "Get a drink at the bar"; telephone → the `.phone-ringing` links with Sam's line; man at the bar (stool 2) → "Approach" / "Approach the artists"; painter (stool 4) → "Sketch him on a napkin"; a figure with no link is not clickable. Photographs, glass, door stay inspections. Figures have a slim invisible hit cylinder. TO THE BAR, the curtain and goIn are gone. Tests: `scratchpad/scene-closeups-2026-09-25/fi_routes.py OUTDIR WIDTH [reduced]` (from `scratchpad/audit-2026-09-16`, `PYTHONPATH=.`, server on 8777): 0 fails at 1280, 390 and reduced; Safari not run. Open for Sam: pink card lines and hotspot names; whether the telephone card should stay while the game's own ringing modal also appears.
...
The visual language was written down first (top of the log) and the survey of fifty passages measured against it. Touches, one commit each: **the colour of the room just left** (UserScript "THE COLOUR OF THE ROOM JUST LEFT" + `.dss-afterglow`: a wash of the previous passage's venue or dream colour arrives with the next page at 32% and fades over 5.5s after a 0.7s hold; `window.DSS_AFTERGLOW = false` off, `DSS_AFTERGLOW_ALPHA` retunes; opacity only so it stays under reduced motion); **one street** (the `street-night` wash now also on every `outdoor` passage that is not the hub, so the dream turn-backs match the returns); **every title an after-image** (`.venue-title` gets the hub title's two misregistered copies, static); **the dream worlds' colours present** (`--dss-dream-tint` alphas roughly doubled to .19–.24, the old values were invisible at play size). Noted for Sam, not changed: the Chippy strip lights and Trisha's neon frame (deliberately loud), the minigame result banners (game-UI voice, words in his source), Courier scene captions vs Playfair hub for the same words.
...
**The Fetch (Ezekiel), same night, 23:20 UTC.** Sam: the Ezekiel stranger is the Fetch, standing just below the way to Centre Point. Built as DOORS `fetch` (`fetch:true`) at c17,r8 in the road of Dean Street: not on the base canvas but in the frame loop, as the player's own sprite (`sprites.down[0]`) with a cold cast and a faint radial paleness, alpha rising with Manhattan distance (nothing within 1.5 tiles, full at 6.5), so it thins as you approach and is gone before you arrive; the tile still fires. No pool in the glow layer. Hidden hub link `The Fetch on Dean Street`, gated on `$ezekielVision is true and $fetchSeen is false`; passage `The Fetch on Dean Street [outdoor]`, pink line, +10 confidence, `[[Walk on|Dean Street]]`. Benito's jump to `Benito Recognises the Wheel` removed from The French; notebook echo for Ezekiel follows `$fetchSeen`. It was first placed at r4 and r7, both under the Chippy's sign or beside its door glyph; r8 clears them. The Diadem Court "A DOORWAY" sign sits just to its right. Test `scratchpad/strangers-2026-09-24/fetch_test.py`. Sam keeps the small `dss-dream-residue` echoes in the regulars' rooms.

**Misty "Morris" Minor (Nazca), 23:50 UTC.** Sam: another poet, whom you think you met with his family in Carthage. DOORS `morris` (`stranger:true, item:'paper'`) at c7,r17, the Wardour Street corner of Bateman Street (the lamp at c7,r14 rules out the north corner). `drawStranger` now branches on `item`: paper in hand for him, brass for Gooch. Hidden hub link `Misty Morris Minor` on `$nazcaTracing is true and $morrisMet is false`; passage `Misty Morris Minor [outdoor]`, pink placeholder from his sketch, +10 confidence, `[[Walk on|Dean Street]]`. Lackland unhooked: the jump in his office removed, the Dean Street "Go to Lackland's Office" gate no longer re-opens for the tracing, notebook Nazca tick follows `$morrisMet`. Generic walk test `scratchpad/strangers-2026-09-24/stranger_test.py <slug> "<seeds>" fromC fromR dir holdMs "<passage>" outdir`.

**Tom Longshanks (Easter Island) and Guilliam Songstrong (Pyramid), 2026-09-25 00:40 UTC.** Longshanks, a contemporary classical composer, DOORS `longshanks` at c7,r29 (Wardour corner of Old Compton, south side; the lamp at c7,r26 rules out the north), `item:'paper'` (a score), lilac pool. Songstrong, a Welsh singer (Sam first said Metsong, then changed it), DOORS `songstrong` at c31,r40 (Shaftesbury corner of Frith, east side; lamp at c28,r40 rules out the west), `item:'none'` (empty hands), warm pool. Hub links on `$easterGlyph`/`$longshanksMet` and `$pyramidNumber`/`$songstrongMet`; passages `Tom Longshanks [outdoor]`, `Guilliam Songstrong [outdoor]`, pink lines, +10 confidence, `[[Walk on|Dean Street]]`. Unhooked: Red's jump (At the Corner of Dean Street and Greek Street) and the Dean Street "Someone's on the corner" gate no longer re-opens for the glyph; Inis's jump (O'Flatterly's shop) and the Dean Street Cecil Court link no longer re-opens for the number (the CLOSED grey sign now shows whenever the page is returned). Notebook ticks follow the two new flags.

All five strangers are built. The five old "recognises" passages remain in the file, unreachable. Open for Sam: the pink lines of all five scenes, and whether Gooch and Songstrong should carry a sound (audio is his). Each needs the same five pieces plus unhooking that world's recognises scene (Red c. 45302, Inis 46176, Lackland 48118, Benito 50368) and its notebook echo variable.
