...
- **Lackland's office music — DONE & wired** (`lacklands-office-music.mp3`, bed `lackland-office`, vol 0.30; plays across the whole den — office `venue-lackland` + basement `venue-lackland-back`; cellar-pump hum removed from Lackland, kept for Copper's cellar). ⚠ **Known loop seam:** a slight pause at the MP3 loop wrap (encoder-priming — the engine only trims it on the first play, not the wrap). Dr Quill judged it **acceptable for now**; the fix when he wants it is a one-off **PCM/WAV re-bounce** (same filename → I re-embed + sync, nothing else changes).
