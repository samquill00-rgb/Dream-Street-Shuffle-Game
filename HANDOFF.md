## The Fetch — visual lift, 2026-09-23

Sam: "a visual lift for the fetch". Same branch `claude/centre-point-scene-9jo9fx`, second commit. Fetch Street SVG only (choreography, timings, figure rig, captions and their wording untouched): the flat pale sky is now a night gradient settling into the mist at the horizon; the ground darkens towards the viewer; the picture's edges feather into the glass pane through a radial mask (`fsMask`) instead of a hard white rectangle (also hides the ink filter's edge artefact); the tower slab is taller and darker with a red aviation light blinking on a 4s loop and a faint reflection in the wet ground; the lamp has a post and head, a stronger pool and a reflection; three faint wet-sheen streaks on the pavement; the frame grows from 340 to 430px wide. The three captions moved out of the `dss-ink` group so they no longer wobble. Reduced motion: the passage script pauses the SVG at 21s (the settled end: he stands small by the tower) and reveals the links after 0.4s instead of 15s. Screenshots `centre-point-review/lift2-*` (after) beside `desktop-02/04-fetch-*` (before).
