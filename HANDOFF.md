...
**Known cosmetic remainders (LOW, deliberate):** rising-number `up()` list omits 3 overlay ids (paused stat-deltas animate under those popups); cig overlay doesn't defer to soft popups (can stack over them); napkin popup closable only via its buttons.
