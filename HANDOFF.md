...
- **Merging:** the branch is 14 commits ahead of main at 05:00 UTC (266dbf6 … f0e58fc) plus this handoff commit. Merge in GitHub Desktop; if `Dream Street Shuffle.html` conflicts, resolve by re-running `python3 sync_html.py` after the merge and committing the result. Nothing else on the branch touches files other assistants edit except the twee's UserScript functions named above and the phone CSS block.
